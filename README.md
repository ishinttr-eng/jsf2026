...
- **出演者**: 日付タブ（すべて/9・12/9・13、「すべて」選択時は両日横断で検索・一覧）。出演者名・かな・部門（awardEntry）で検索（カタカナ/ひらがな同一視）、会場・ジャンル絞り込みは専用ドロップダウンで（ジャンルはテキスト検索と機能が重複するため対象外）、☆でお気に入り登録。カードタップでアーティスト詳細（紹介文・ジャンル・地域・U-25・部門エントリー等）を表示
