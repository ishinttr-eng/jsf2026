...
- **演目ごとの天気**: 出演者カード（演奏中・出演者・マイタイムテーブル共通）に、その演目の開始時間帯（1時間単位）の天気予報を小さなバッジで表示。Open-Meteo（APIキー不要）から開催日（9/12・9/13）の時間別データを取得し、演目の`startMin`から該当時間を引いて表示する（全会場が徒歩圏内のため会場ごとではなく1地点分のみ）
- **出演者**: 日付タブ（すべて/9・12/9・13、「すべて」選択時は両日横断で検索・一覧）。出演者名・かな・部門（awardEntry）で検索（カタカナ/ひらがな同一視）、会場・ジャンル絞り込みは専用ドロップダウンで（ジャンルはテキスト検索と機能が重複するため対象外）、☆でお気に入り登録。カードタップでアーティスト詳細（紹介文・ジャンル・地域・U-25・部門エントリー等）を表示。部門エントリーがある演目には公式サイトの投票ページ（該当演目を指定した`?jsf-plan=...#myplan`リンク）への導線を表示（投票自体はGPSでの会場内判定が必要な公式独自の仕組みのため、リンクのみ）
- **マップ**: 全50会場＋タイアップステージ（T1〜T15・未来応援ステージJ、レギュラーとは色・形を分けて表示）をLeaflet + OpenStreetMapのピンで表示。ホバーで会場名ツールチップ、ピンタップで演奏中/次の出演者・徒歩時間を確認。会場間・現在地からのルートを地図上に描画（事前計算済みの実測ルートがあれば実線、無ければ直線の概算を破線で表示。いずれも白フチ＋高彩度カラーでタイル配色に埋もれないように）。「マイルートモード」では現在時刻に応じて次に向かうべきお気に入り会場への経路を自動表示。Googleマップの徒歩ナビへのリンクも常に併記（APIキー不使用）。ピンから会場のタイムテーブルを開くと、演奏中の項目まで自動スクロール
- **マイタイムテーブル**: お気に入りから自分のタイムテーブルを生成。表示日は演奏中の時刻シミュレーションに自動追従。演奏中/次はここを自動でハイライト＆スクロール。会場間の移動時間・時間重複を警告し、タップで地図上のルート表示に飛べる。スケジュール表モードでは日付・表示モードのタブを上部に固定表示（縦スクロール中も常に見える）
- **お気に入りの共有・バックアップ**（設定画面から）: 🔗共有リンクの作成（`?fav=...`形式のURLをコピー、開いた側は取り込み前に内容を確認するプロンプトが出る）／⬇️ファイルへの書き出し／⬆️ファイルからの読み込み（追加または置き換え）に対応。機種変更や友だちとの共有に利用可能。[公式サイト](https://www.j-streetjazz.com/event/35th/performers/#myplan)の「予定を共有する」リンク（`?jsf-plan=...`形式、出演IDがそのまま一致するため変換不要）から開いた場合も同じ取り込みプロンプトが出る
